...
void ParallelPrimes(unsigned sizeOfFlags);
...
int main(int argc, char **argv) {
  ...
  if (P == 1) {
    ...
    ParallelPrimes(flagSize);
     // find last prime
     while(!flags[flagSize]  )
     {
       flagSize--;
     }
     lastPrime = flagSize*2 +3;
  }
...
void ParallelPrimes(unsigned sizeOfFlags) {
  ...
  int thread_count = omp_get_num_threads();
    ...
    for (i = my_rank; i < sizeOfFlags; i+= thread_count) {    /* For every odd number */
      prime = 2*i + 3;
